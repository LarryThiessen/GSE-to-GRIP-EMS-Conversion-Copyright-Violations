...
function GE.BuildExportList()
    ...
    for seqName, entry in pairs(Engine.sequences) do
        if entry and entry.data then
            ...
            -- type(), not truthiness. BuildExportList walks pairs(Engine.sequences)
            -- -- EVERY stored sequence -- and it is the first thing the export
            -- dialog calls, so one malformed stored entry previously failed the
            -- whole dialog, not just its own row. Measured: a number or boolean
            -- steps raised "attempt to get length of field 'steps'" right here;
            -- a string steps CLEARS the length operator (it fabricates a count
            -- from the character length) and instead dies four lines down at the
            -- ipairs in the version scan. Both sites need the same guard.
            ...
            if seqData.versions and type(seqData.versions) == "table" then
                for _, v in ipairs(seqData.versions) do
                    -- The string shape reaches this ipairs alive, having cleared
                    -- the length operator above; type() is what stops it.
                    if v and type(v.steps) == "table" then
            ...
            -- Fallback: legacy flat steps. Same guard family, different field:
            -- the sequence-level steps rather than the version-level one. It was
            -- masked by the version-scan raise above and starts raising here once
            -- that is closed, so it is guarded in the same pass.
            if type(seqData.steps) == "table" then
            ...
            if seqData.versions and type(seqData.versions) == "table" then
                for _, v in ipairs(seqData.versions) do
                    -- Third site on the same field in the same function. The
                    -- ipairs in the variable scan above raised first and masked
                    -- this one; guarding only the first two moves the raise here.
                    if v and type(v.steps) == "table" then
                        for _, step in ipairs(v.steps) do
                            if type(step) == "string" then
            ...
            -- Fallback: legacy flat steps -- second reader of the same field.
            if type(seqData.steps) == "table" then
                for _, step in ipairs(seqData.steps) do
                    if type(step) == "string" then
                        for refName in step:gmatch(clickPattern) do
...
--
-- MEASURED REMAINDER -- now CLOSED, and its stated reason was wrong. Kept as
-- the record so the next sweep reads this instead of re-deriving it.
--
-- Pass 13 guarded the stored-steps readers in GE.BuildExportList above and
-- left four behind bare truthiness. All four are type() == "table" as of this
-- pass:
--   GE.ResolveVariableDeps        :266 (ver.steps), :278 (flat seqData.steps)
--   GE.DetectMissingEmbeddedDeps  :394 (ver.steps), :410 (flat seqData.steps)
-- Each guard is followed within one or two lines by an ipairs over the same
-- field. ipairs raises on a number, a boolean AND a string, so a length
-- operator is not an adequate model here: the string shape clears a length
-- check and never clears an ipairs. Verified on Lua 5.1.5.
--
-- WHY THE DEFERRAL WAS WRONG. Pass 13 recorded the reason as "both run only
-- over the sequences the user SELECTED, not over every stored one". That is
-- true of GE.DetectMissingEmbeddedDeps, which genuinely has one caller, and
-- false of GE.ResolveVariableDeps, which has five -- one true statement about
-- one function generalised to both. Enumerated callers:
--   GE.BuildExportList            1  ExportFrame.lua:620
--   GE.DetectMissingEmbeddedDeps  1  ExportFrame.lua:209
--   GE.ResolveVariableDeps        5  ExportFrame.lua:188   export dialog,
--                                       the only multi-select caller
--                                    SequenceList.lua:909  right-click
--                                       context-menu Export on ONE row
--                                    SequenceEditor.lua:450  the editor's
--                                       Export button
--                                    Core.lua:809  the /gems export slash
--                                       command, which does not nil-check GE
--                                    PublicAPI.lua:259  the third-party
--                                       plugin snapshot builder
-- So the deferred function had the WIDER caller surface, not the narrower one.
--
-- And pass 13 is what made these four sites reachable by a normal user
-- action. Before it, a malformed stored entry raised inside BuildExportList
-- and the export dialog never opened, so nothing downstream could run. With
-- the dialog opening and the bad row rendering, the raise MOVED to whatever
-- the user touches next -- a right-click Export, the editor's Export button,
-- a slash command, or a plugin read.
---------------------------------------------------------------------------
        ...
        if entry and entry.data then
            ...
            if seqData.versions and type(seqData.versions) == "table" then
                for _, ver in ipairs(seqData.versions) do
                    if ver and type(ver.steps) == "table" then
...
function GE.ExportCollection(seqNames, varNames, macNames, exportMeta)
    ...
    -- Do-not-share refusal. Whole-selection, not per-sequence: silently dropping
    -- the marked member would hand the user a collection string that is not the
    -- selection they asked for, and the envelope's strictest-privacy rollup
    -- already treats the selection as one unit.
    local blockedName = GE.FindNoRedistribute(seqNames)
    if blockedName then
        return false, string.format(L["GEMS_EXPORT_NO_REDISTRIBUTE"], blockedName)
    end
...
function GE.SerializeWithProvider(providerId, seqNames, varNames, macNames, exportMeta)
    ...
    -- Same refusal as the native rail, and it must be here rather than only in
    -- GE.ExportCollection: this entry point builds the COLLECTION payload
    -- directly and hands it to third-party provider code, so a gate on the
    -- native encoder alone would leave the plugin rail wide open.
    local blockedName = GE.FindNoRedistribute(seqNames)
    if blockedName then
        return false, string.format(L["GEMS_EXPORT_NO_REDISTRIBUTE"], blockedName)
    end
