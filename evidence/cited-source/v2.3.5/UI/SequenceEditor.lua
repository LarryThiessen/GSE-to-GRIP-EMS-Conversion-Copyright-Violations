...
function SE:Init(parentPanel)
    ...
    kpRow:SetHeight(90)
    kpRow:SetPoint("TOPLEFT", resetRow2, "BOTTOMLEFT", 0, -4 - UI:GetRowGap())
    ...
    versionLiveBtn:SetScript("OnClick", function(self)
        ...
        if not lbEntry or not lbEntry.data or not lbEntry.data.versions then
            return
        end
...
function SE:UpdatePreview()
    ...
    if not previewVer or not previewVer.steps or #previewVer.steps == 0 then
        SE.previewFrame:Hide()
