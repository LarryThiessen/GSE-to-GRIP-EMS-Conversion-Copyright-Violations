...
function MCB:Initialize()
    ...
    if GRIPEMS.RegisterCallback then
        ...
        -- comment at :76-77: a string method "leads to self[method](self, ...)"
        -- while a function ref "leads to functionref(...)". Only the string form
        -- gets self.
        --
        -- All three arms carried function(_, _, varName) and read one position
        -- late. VARIABLE_UPDATED fires (name, value) at Data/VariableStore.lua:238,
        -- :346 and :935, so varName bound the VALUE and _Invalidate dropped the
        -- wrong cache slot while the real one survived. VARIABLE_CREATED and
        -- VARIABLE_DELETED fire (name) ONLY (:235, :265, :344, :345), so varName
        -- bound NIL and _Invalidate returned on its own "if not varName" guard --
        -- the bake cache was never invalidated on create or delete at all.
        GRIPEMS.RegisterCallback(MCB, "VARIABLE_UPDATED", function(_, varName)
            MCB._Invalidate(varName)
        end)
        GRIPEMS.RegisterCallback(MCB, "VARIABLE_CREATED", function(_, varName)
            MCB._Invalidate(varName)
        end)
        GRIPEMS.RegisterCallback(MCB, "VARIABLE_DELETED", function(_, varName)
            MCB._Invalidate(varName)
        end)
    end
