...
WN.currentNotes = {
    version = "2.3.16",
    title = "v2.3.16 -- Imported Sequences Keep Their Author",
    sections = {
        {
            heading = "Fixed",
            items = {
                "Saving an imported sequence no longer puts your name on it. A sequence in the older signature format upgraded itself the first time you saved, and that upgrade wrote your identity into the original-author field. When the import arrived with no signature there was nothing to upgrade, but it ran regardless and your byline landed on someone else's sequence. An unsigned import stays unsigned now, in both the main editor and the popup editor.",
                'An imported sequence with no provenance field used to show the amber "Damaged" badge. All three import paths defaulted that field to a value the verifier does not recognise, so a third-party sequence carrying no provenance read as corrupt instead of merely unsigned. It reads "Provenance unknown" now.',
            },
        },
        {
            heading = "Added",
            items = {
                "The provenance badge on an imported sequence says what it means. Hover it and the verification state is spelled out in a sentence, for all six states: Verified, Modified, Tampered, Damaged, Forked, and Provenance unknown.",
