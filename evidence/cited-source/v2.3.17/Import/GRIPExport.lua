...
--- Is this sequence marked do-not-share by its author?
--- seqData.noRedistribute is set at import time from the inbound payload
--- (Import/LegacyImport.lua LI.ResolveNoRedistribute). It is a separate axis
--- from privacyMode, which only decides how the author identity is SCRUBBED and
--- never refuses an export.
--- @param seqData table|nil Stored sequence data
--- @return boolean marked
function GE.IsNoRedistribute(seqData)
    return type(seqData) == "table" and seqData.noRedistribute and true or false
end

--- Find the first do-not-share sequence in a selection.
--- Shared by every export entry point so one refusal rule covers the single,
--- collection, provider and text rails rather than four drifting copies.
--- @param seqNames table|nil Array of sequence names
--- @return string|nil name First marked sequence name, nil when none are marked
function GE.FindNoRedistribute(seqNames)
    if type(seqNames) ~= "table" then
        return nil
    end
    local Engine = GRIPEMS.Engine
    local store = Engine and Engine.sequences
    if not store then
        return nil
    end
    for _, seqName in ipairs(seqNames) do
        local entry = store[seqName]
        if entry and GE.IsNoRedistribute(entry.data) then
            return seqName
        end
    end
    return nil
...
function GE.Export(sequenceName)
    ...
    -- Do-not-share refusal, ahead of every build step: no payload table, no
    -- encoded string, nothing for a caller to salvage. This is the choke point
    -- the P2P send path funnels through as well.
    if GE.IsNoRedistribute(entry.data) then
        return false, string.format(L["GEMS_EXPORT_NO_REDISTRIBUTE"], sequenceName)
    end
...
function GE.ExportAsText(seqNames, varNames, macNames)
    ...
    for si, seqName in ipairs(seqNames) do
        ...
        -- A do-not-share sequence is refused on this rail too. This one returns
        -- a plain string rather than an (ok, message) pair, so the refusal is
        -- emitted IN PLACE of the sequence body: the user still sees which
        -- sequence was withheld and why, and no step text is rendered.
        if entry and entry.data and GE.IsNoRedistribute(entry.data) then
            lines[#lines + 1] = string.format(L["GEMS_EXPORT_NO_REDISTRIBUTE"], seqName)
            lines[#lines + 1] = ""
        elseif entry and entry.data then
            local seqData = entry.data
