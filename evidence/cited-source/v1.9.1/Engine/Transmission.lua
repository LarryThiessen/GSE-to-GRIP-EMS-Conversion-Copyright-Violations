...
-- Pending send dialog sequence name (set before StaticPopup_Show)
...
function T:Initialize()
    ...
    StaticPopupDialogs["GRIPEMS_SEND_REPAIR_WARN"] = {
        text = "",
        button1 = "",
        button2 = CANCEL,
    ...
    if not skipRepairCheck then
        ...
        if RA then
            ...
            if seqEntry and seqEntry.data then
                ...
                if iCount > 0 then
                    local dialog = StaticPopupDialogs["GRIPEMS_SEND_REPAIR_WARN"]
                    ...
                    StaticPopup_Show("GRIPEMS_SEND_REPAIR_WARN")
                    return
                end
            end
        end
...
function T:SendMetaRequest(seqName, target)
    ...
    GRIPEMS:Debug(string.format(L["GEMS_META_SENT"], seqName))
end
...
function T:SendSpellCache()
    ...
    -- Build lightweight name -> spellID map
    local cachePayload = {}
...
function T:HandleReceive(prefix, message, channel, sender)
    ...
    elseif cmd == D().CMD_META then
        local seqName = decoded.SeqName
        local remoteUpdatedAt = decoded.UpdatedAt or 0
        if not seqName then
            return
        end
        ...
        local localUpdatedAt = localData and localData.updatedAt or 0

        if not localEntry or localUpdatedAt < remoteUpdatedAt then
            GRIPEMS:Debug(string.format(L["GEMS_META_NEWER"], seqName, sender))
            T:SendRequest(seqName, sender)
        else
            GRIPEMS:Debug(string.format(L["GEMS_META_CURRENT"], seqName, sender))
        end
    ...
    elseif cmd == D().CMD_SPELLCACHE then
        ...
        if SC and SC.MergeRemoteCache then
            ...
            if merged > 0 then
                GRIPEMS:Debug(string.format(L["GEMS_SPELLCACHE_MERGED"], merged, sender))
            end
        end
...
function T:ShowSendDialog(seqName)
    ...
    StaticPopupDialogs["GRIPEMS_SEND_SEQUENCE"].text = text
