...
SF.Sequential = {
    ...
    BuildClickBody = function()
        local body = [=[
            ...
            if self:GetAttribute("holdOnChannel") == "1" and PlayerIsChanneling() then
                self:SetAttribute("macro", nil)
                self:SetAttribute("unit", nil)
                local rel = ""
                if self:GetAttribute("releaseAtMax") == "1" then
                    rel = self:GetAttribute("empowerRelease") or ""
                end
                self:SetAttribute("macrotext", rel)
                self:CallMethod("PostClick")
                return
            end
