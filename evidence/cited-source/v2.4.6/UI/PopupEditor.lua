...
function PE:CreateInstance(index)
    ...
    versionBtn:SetScript("OnClick", function(self)
        ...
        -- type(), not truthiness: the "#entry.data.versions" loop below raises
    ...
    -- Tree-lock notice: says why the step rows are inert when the active
    -- version's action tree drives its steps. Hidden until PE:RefreshStepList
    -- finds the lock (see PE:IsTreeLocked).
    local treeLockNotice = contentArea:CreateFontString(nil, "OVERLAY")
    UI:SetFont(treeLockNotice, 10)
    treeLockNotice:SetPoint("TOPLEFT", contentArea, "TOPLEFT", 8, -4)
    treeLockNotice:SetPoint("TOPRIGHT", contentArea, "TOPRIGHT", -8, -4)
    treeLockNotice:SetJustifyH("CENTER")
    treeLockNotice:SetWordWrap(true)
    treeLockNotice:SetText(L["GEMS_UI_POPUP_TREE_READONLY"])
    treeLockNotice:SetTextColor(C.textWarning:GetRGBA())
    treeLockNotice:Hide()
    inst.treeLockNotice = treeLockNotice
...
InitPopupStepRow = function(row, data, inst)
    ...
    -- A version whose action tree is live compiles its own steps, so flat editing
    -- here cannot be persisted (see PE:IsTreeLocked). Compute the lock once and
    -- apply it to both writable widgets on the row. Enable/Disable, not
    -- SetEnabled: those are the canonical EditBox methods.
    local treeLocked = PE:IsTreeLocked(inst)
    if treeLocked then
        row.editBox:Disable()
    else
        row.editBox:Enable()
    end
    row.spellPickerBtn:SetEnabled(not treeLocked)
...
--- Does this version's action tree drive its steps?
--- ONE expression, two callers (PE:IsTreeLocked and PE:SaveSequence). Do not
--- inline a second copy: a duplicated predicate gets fixed at one site only.
--- @param ver table|nil A sequence version
--- @return boolean
local function TreeDrivesSteps(ver)
    return type(ver) == "table" and type(ver.actions) == "table" and #ver.actions > 0 and not ver.importBakedSteps
end

--- Can the popup's flat step editing be persisted for the active version?
--- The popup edits FLAT steps only (v1.2.0 S14-B; tree editing is S14-C). When a
--- version's action tree is live, steps is a compiled artifact of actions:
--- Engine:UpdateSequenceData recompiles it from the tree on the way through, so a
--- flat edit saved here is discarded inside the save that made it. The structured
--- editor never hits this because it compiles steps FROM its working tree and
--- clears importBakedSteps; the popup has no tree editor to compile from.
--- @param inst table Popup instance
--- @return boolean true when flat step editing cannot be persisted
function PE:IsTreeLocked(inst)
    if not inst or not inst.currentSequence then
        return false
    end
    local engine = GRIPEMS.Engine
    local entry = engine and engine.sequences and engine.sequences[inst.currentSequence]
    local seqData = entry and entry.data
    if not seqData or type(seqData.versions) ~= "table" then
        return false
    end
    return TreeDrivesSteps(seqData.versions[inst.activeVersionIndex])
end
...
function PE:RefreshStepList(inst)
    ...
    -- Guarded: an instance created before this change carries no notice widget.
    if inst.treeLockNotice then
        if PE:IsTreeLocked(inst) then
            inst.treeLockNotice:Show()
        else
            inst.treeLockNotice:Hide()
        end
    end
end
...
function PE:LoadSequence(inst, seqName)
    ...
    local ver = type(seqData.versions) == "table" and seqData.versions[inst.activeVersionIndex] or nil
    if not ver then
        ver = type(seqData.versions) == "table" and seqData.versions[1] or nil
...
function PE:SaveSequence(inst)
    ...
    if type(oldData.versions) == "table" then
        ...
        for i, ver in ipairs(oldData.versions) do
            newData.versions[i] = {
                ...
                -- Mirror of the same hand-enumerated rebuild in UI/SequenceEditor.lua.
                -- This file carried ZERO occurrences of holdOnChannel before this fix,
                -- so the popup save dropped both fields exactly as the main editor did.
                -- Fixing only the loud path would have left the silent one.
            ...
            -- Compiled labels are display metadata for compiled or baked steps, and
            -- they always carry through a save as a fresh sparse copy (pairs, not
            -- ipairs). Where a live tree exists the recompile in
            -- Engine:UpdateSequenceData overwrites them, so carrying them costs
            -- nothing there. Where importBakedSteps is set that recompile is SKIPPED,
            -- so this carry is the ONLY thing preserving them and dropping it silently
            -- demotes the version to its stepLabels: Engine:BuildExecLabels reads
            -- (compiledLabels or stepLabels). The popup-save-field-carry gate in the
            -- Hub repo asserts this contract; do not narrow it without changing that.
            ...
            if i == activeIdx then
                -- Active version: use working steps and step function, UNLESS the
                -- action tree drives them. A locked version takes steps and
                -- stepFunction from stored data because Engine:UpdateSequenceData
                -- recompiles both from the tree inside this very call, so a flat
                -- edit written here is discarded by the save that wrote it. The UI
                -- blocks the edit at source (PE:IsTreeLocked disables the row
                -- widgets and gates every mutator); this is the guard that holds if
                -- any path reaches the save anyway.
                local locked = TreeDrivesSteps(ver)
                if not locked then
                    newData.versions[i].stepFunction = inst.workingStepFunction or ver.stepFunction
                end
                for j, step in ipairs((locked and ver.steps) or inst.workingSteps or {}) do
    ...
    -- This save just re-signed everything, so a quick-edit re-sign armed by the
    -- structured editor on the same sequence would only append a duplicate
    -- modifier-chain entry. UI/SequenceList.lua does the same reach-in on rename.
    local SEd = GRIPEMS.SequenceEditor
    if SEd and SEd._CancelQuickResign then
        SEd:_CancelQuickResign(inst.currentSequence)
    end
    ...
    if savedEntry and savedEntry.data then
        local savedVer = type(savedEntry.data.versions) == "table" and savedEntry.data.versions[inst.activeVersionIndex]
            or nil
        if savedVer then
...
function PE:RefreshVersionBar(inst)
    ...
    local entry = engine.sequences[inst.currentSequence]
    -- type(), not truthiness: the "#entry.data.versions" read below raises
    -- "attempt to get length of field 'versions'" on a scalar.
    if not entry or not entry.data or type(entry.data.versions) ~= "table" then
        return
    end
...
function PE:SwitchVersion(inst, idx)
    ...
    inst.isDirty = false
    -- The undo stack is part of the state this discards. Its closures capture the
    -- OUTGOING version's step indices, and the working copy is replaced below, so
    -- an undo fired after the switch rewrites the INCOMING version's steps at the
    -- outgoing version's positions. PE:LoadSequence and PE:HideAndRelease already
    -- clear it on their own state transitions; this was the third one that did not.
    if inst.undoStack then
        inst.undoStack:Clear()
    end
    ...
    local ver = type(entry.data.versions) == "table" and entry.data.versions[idx] or nil
    if not ver then
        return
    end
