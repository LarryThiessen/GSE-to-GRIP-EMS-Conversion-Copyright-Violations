...
function GE.PrepareExportVersions(seqData)
    ...
    if seqData.versions and type(seqData.versions) == "table" then
        for vi, ver in ipairs(seqData.versions) do
            ...
            if ver.stepLabels and verCopy.steps then
                local labelsCopy = {}
...
function GE.Export(sequenceName)
    ...
    local ilVer = seqData.versions and seqData.versions[seqData.defaultVersion or 1]
    if
        ilAC
...
local function emitActionTree(actions, lines, indent)
    for idx, node in ipairs(actions) do
        ...
        elseif t == D.ACTION_TYPE_IF then
            lines[#lines + 1] = indent .. idx .. ") " .. pfx .. "If " .. (node.variable or "")
            if node.children then
                if node.children[1] then
                    lines[#lines + 1] = indent .. "  True:"
                    emitActionTree(node.children[1], lines, indent .. "    ")
                end
                if node.children[2] and #node.children[2] > 0 then
...
local function FormatVersionBlock(ver, vIdx, vCount, lines)
    ...
    if ver.keyPress and ver.keyPress ~= "" then
        lines[#lines + 1] = L["GEMS_EXPORTTEXT_KEYPRESS_LABEL"] .. ver.keyPress
    end

    -- KeyRelease (inline, skip if empty)
    if ver.keyRelease and ver.keyRelease ~= "" then
        lines[#lines + 1] = L["GEMS_EXPORTTEXT_KEYRELEASE_LABEL"] .. ver.keyRelease
    end
end
    ...
    for si, seqName in ipairs(seqNames) do
        ...
        if entry and entry.data then
            ...
            lines[#lines + 1] = L["GEMS_EXPORTTEXT_AUTHOR_LABEL"]
                .. (seqData.author or L["GEMS_EXPORTTEXT_AUTHOR_UNKNOWN"])
            if seqData.originalAuthor and seqData.originalAuthor ~= "" then
