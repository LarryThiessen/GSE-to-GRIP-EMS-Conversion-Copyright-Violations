...
function Engine:ActivateSequence(name, sequenceData)
    ...
    local function doActivate()
        ...
        btn.PostClick = function(self)
            ...
            -- Accessibility: speak step on advance (Phase 1C + 1C-bis label-first)
            -- Fallback chain (Phase 1C-bis-outline):
            --   1) compiledLabels[idx]  -- outline mode, from ActionCompiler
            --   2) stepLabels[idx]      -- flat mode sidecar
            --   3) ACCESS_SPEECH_STEP_FORMAT -- generic "Step X of Y"
            if GRIPEMS.Speech and GRIPEMS.Speech.Announce then
                local newStep = tonumber(self:GetAttribute("step")) or 1
                local speechVer = GRIPEMS.Engine:GetActiveVersion(self.seqData)
                local numSteps = speechVer and speechVer.steps and #speechVer.steps or 1
                local compiledLabels = speechVer and speechVer.compiledLabels
                local stepLabels = speechVer and speechVer.stepLabels
                local label = (compiledLabels and compiledLabels[newStep]) or (stepLabels and stepLabels[newStep])
                    ...
                    if not effectiveSteps or #effectiveSteps == 0 then
                        ...
                        variantBtn.PostClick = function(self)
                            ...
                            if GRIPEMS.Speech and GRIPEMS.Speech.Announce then
                                ...
                                local compiledLabels = speechVer and speechVer.compiledLabels
                                local stepLabels = speechVer and speechVer.stepLabels
                                local label = (compiledLabels and compiledLabels[newStep])
                                    or (stepLabels and stepLabels[newStep])
                                if label and label ~= "" then
                                    GRIPEMS.Speech:Announce(label)
                                else
...
function Engine:AdvanceStep(name)
    ...
    local function doAdvance()
        ...
        local numSteps = ver and ver.steps and #ver.steps or 0
        if numSteps == 0 then
            return
        end
...
function Engine:GetSequenceList()
    ...
    for name, entry in pairs(self.sequences) do
        ...
        table.insert(list, {
            name = name,
            stepCount = ver and ver.steps and #ver.steps or 0,
