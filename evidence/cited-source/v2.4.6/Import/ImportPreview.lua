...
local function AttachProvenance(entry, seqObj)
    ...
    local prov = {
        -- A converted payload carries a source version stamp but no authorship
        -- block, so the byline the stamp implies is the one to surface. An
        -- explicit non-empty originalAuthor always wins; the fallback is gated
        -- on the stamp so an unstamped payload's plain author is never promoted
        -- to original author. Unlike seqData.author in BuildSeqDataFromGRIP1,
        -- seqObj.author here is the RAW wire field and may be nil, so the
        -- trailing "" is what guarantees a string for the UI.
        originalAuthor = (seqObj.originalAuthor and seqObj.originalAuthor ~= "" and seqObj.originalAuthor)
            or (seqObj.gseVersion and seqObj.author)
            or "",
        ...
        -- Preview runs BEFORE commit, so the source stamp is the only origin
        -- signal a converted payload has here; read it so the existing
        -- provenance badge resolves in the preview tooltip rather than staying
        -- dark until the sequence is committed. An explicit inbound
        -- provenanceSource still wins, and "" remains the final default.
        provenanceSource = seqObj.provenanceSource or (seqObj.gseVersion and "gse-legacy") or "",
        lastModifier = seqObj.lastModifier or "",
        -- Inbound do-not-share, resolved from the same three wire shapes the
        -- commit path uses (LegacyImport.lua LI.ResolveNoRedistribute). Surfaced
        -- on the provenance block so the preview can badge it BEFORE the user
        -- commits, rather than the flag first becoming visible once the
        -- sequence is already stored.
        noRedistribute = LI.ResolveNoRedistribute and LI.ResolveNoRedistribute(seqObj) or nil,
    ...
    if type(decoded) == "table" and decoded.type == "COLLECTION" then
        ...
        for seqName, seqValue in pairs(payload.Sequences) do
            ...
            elseif type(seqValue) == "table" and seqValue.GSEDeltaFork then
                ...
                -- list, not the sequence itself. Refuse instead of previewing it,
                -- which would silently show an empty sequence.
        ...
        if payload.Variables and type(payload.Variables) == "table" then
            for varName, varValue in pairs(payload.Variables) do
                ...
                elseif type(varValue) == "table" and varValue.GSEDeltaFork then
                    -- Linked copy of protected content: a reference plus a change
                    -- list, not the variable itself. Refuse instead of previewing it,
                    -- which would silently show an empty variable.
                    result.warnings[#result.warnings + 1] = "Variable '"
                        .. tostring(varName)
        ...
        if payload.Macros and type(payload.Macros) == "table" then
            for macName, macValue in pairs(payload.Macros) do
                if type(macValue) == "string" then
                    ...
                    if mOk and type(mDecoded) == "table" then
                        ...
                        if mFormat == "GSE3_ENCRYPTED" then
                            result.warnings[#result.warnings + 1] = "Macro '"
                                .. tostring(macName)
                                .. "': "
                                .. tostring(mDecoded)
                        else
                ...
                elseif type(macValue) == "table" and macValue.GSEDeltaFork then
                    -- Linked copy of protected content: a reference plus a change
                    -- list, not the macro itself. Refuse instead of previewing it,
                    -- which would silently show an empty macro.
                    result.warnings[#result.warnings + 1] = "Macro '"
                        .. tostring(macName)
                        .. "': This is a linked copy of protected content and GRIP-EMS can't import it. "
                        .. "It stores a reference to content held by the source sequencer plus a list "
    ...
    if decoded.type == "COLLECTION" then
        ...
        for seqName, seqPayload in pairs(safeSequences) do
            ...
            local colEntry = {
                ...
                -- Inbound do-not-share. Same resolver as the legacy preview row
                -- and the commit path; a COLLECTION member is exactly as able to
                -- carry the flag as a single-sequence payload.
                noRedistribute = LI.ResolveNoRedistribute and LI.ResolveNoRedistribute(seqPayload) or nil,
            }
            AttachProvenance(colEntry, seqPayload)
            colResult.sequences[#colResult.sequences + 1] = colEntry
            if colEntry.noRedistribute then
                colResult.warnings[#colResult.warnings + 1] = string.format(L["GEMS_EXPORT_NO_REDISTRIBUTE"], seqName)
            end
        end
...
function LI._PreviewAddSequence(result, seqName, sequence)
    ...
    local legacyEntry = {
        ...
        freeformCount = freeformCount,
        -- Inbound do-not-share, read from the SAME resolver the commit path
        -- uses so the row the user approves and the sequence that lands agree.
        noRedistribute = LI.ResolveNoRedistribute and LI.ResolveNoRedistribute(sequence) or nil,
    }
    AttachProvenance(legacyEntry, sequence)
    result.sequences[#result.sequences + 1] = legacyEntry

    -- Surface it BEFORE the commit. The flag changes what the user may do with
    -- the sequence afterwards (export and P2P sharing are refused), so it may
    -- not arrive silently and only be discovered at the first blocked share.
    if legacyEntry.noRedistribute then
        result.warnings[#result.warnings + 1] = string.format(L["GEMS_EXPORT_NO_REDISTRIBUTE"], seqName)
    end
...
local function BuildSeqDataFromGRIP1(seq, name)
    local seqData = {
        ...
        -- Inbound do-not-share. Same resolver, same three wire shapes and same
        -- precedence as the LI.ProcessSequence twin, and assigned here in the
        -- constructor so neither provenance arm below has to re-decide it.
        noRedistribute = LI.ResolveNoRedistribute and LI.ResolveNoRedistribute(seq) or nil,
    }
    ...
    if seq.originalAuthor and seq.originalAuthor ~= "" then
        ...
        -- Byte-identical branch to the LegacyImport.ProcessSequence twin
        -- (LegacyImport.lua L914-L935); these two must not diverge.
        if seq.provenanceSource then
            seqData.provenanceSource = seq.provenanceSource
        elseif seq.gseVersion then
    ...
    elseif seq.gseVersion then
        -- A payload converted from a legacy format carries that format's
        -- version stamp forward on the sequence but no authorship block, so
        -- the arm above never fires for it and the sequence used to reach
        -- the Core.lua repair pass unattributed. The stamp IS the origin
        -- record, so synthesize the authorship fields from it here, exactly
        -- as LI.ProcessSequence already does on the raw legacy rail
        -- (LegacyImport.lua L940-L954).
        --
        -- Deliberate difference from that twin on the originalAuthor line:
        -- the twin falls back with a plain `or`, which works there because
        -- its seqData.author may be nil. Here seqData.author is built at
        -- L1136 as `seq.author or ""`, so it is ALWAYS a string, and an
        -- empty string is TRUTHY in Lua -- a plain `or` could never fire and
        -- an authorless payload would store "" as its original author. The
        -- `~= ""` test below is required, not stylistic; do not "restore
        -- parity" with the twin.
        --
        -- The placeholder TEXT also differs from the twin, and that difference
        -- is required too. The branding gate exempts LegacyImport.lua wholesale
        -- on a path match, so its byline predates the rule and was never
        -- scanned. This file IS scanned, and this value reaches the provenance
        -- tooltip as a rendered byline, so it may not name the source product.
        seqData.originalAuthor = (seqData.author ~= "" and seqData.author) or "Unknown (legacy import)"
        seqData.originalAuthorIdentity = ""
        seqData.originalAuthorRealm = ""
        seqData.originalAuthorBattleTag = nil
        seqData.originalCreatedAt = 0
        -- Empty signature => VerifySequence takes its unsigned branch, where
        -- "gse-legacy" is whitelisted alongside "no-provenance" and both
        -- return "unknown" (Identity.lua L298-L307). The modifier chain is
        -- not consulted in that branch, so neither the reset nor the entry
        -- appended below can move a verdict.
        seqData.originalSignature = ""
        seqData.modifierChain = {}
        -- The stamp only supplies the DEFAULT. An explicit inbound
        -- provenanceSource wins, which is what the sibling arm above does at
        -- L1225-L1231 and what AttachProvenance does at L82 -- all three sites
        -- agree that a producer which states its own origin is believed, and
        -- the stamp answers only for a payload that does not.
        --
        -- The coupling that comes with believing the producer: VerifySequence
        -- (Identity.lua L298-L307) whitelists exactly four values, and an
        -- unsigned sequence carrying anything else falls past the whitelist and
        -- verifies as "damaged" rather than "unknown". So a NEW provenance value
        -- a producer starts sending has to be added to that whitelist in the
        -- same release window as the producer change, not after it.
        seqData.provenanceSource = seq.provenanceSource or "gse-legacy"
        seqData.privacyMode = seq.privacyMode or "public"
    ...
    -- Absent provenance and no source stamp: leave fields nil; the one-shot
    -- repair pass in Core/Core.lua stamps the no-provenance family on next load.
