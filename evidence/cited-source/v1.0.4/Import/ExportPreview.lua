...
function GE.BuildExportList()
    ...
    table.sort(result.sequences, function(a, b) return a.name < b.name end)
    ...
    table.sort(result.variables, function(a, b) return a.name < b.name end)
...
function GE.ResolveVariableDeps(selectedSeqNames)
    if not selectedSeqNames then return {} end
                ...
                for _, ver in ipairs(seqData.versions) do
                    if ver and ver.steps then
                        for _, step in ipairs(ver.steps) do
                            if type(step) == "string" then
                                for refName in step:gmatch(clickPattern) do
                                    if not nameSet[refName]
                                        and not checked[refName] then
                                        checked[refName] = true
                                        warnings[#warnings + 1] = string.format(
                                            L["GEMS_EXPORT_PICK_EMBEDDED_WARN"],
                                            seqName, refName)
                                    end
                                end
                            end
...
function GE.ExportCollection(seqNames, varNames)
    ...
    for _, seqName in ipairs(seqNames) do
        ...
        if entry and entry.data then
            ...
            -- Deep-copy versions and translate spell names to ID tags
    ...
    if varNames then
        for _, varName in ipairs(varNames) do
            ...
            if varDef then
                ...
                local cleaned = {}
                if varDef.funct then cleaned.funct = varDef.funct end
                if varDef.events then cleaned.events = varDef.events end
                if varDef.description then cleaned.description = varDef.description end
                if varDef.disabled then cleaned.disabled = varDef.disabled end
                if varDef.author then cleaned.author = varDef.author end
                if varDef.version then cleaned.version = varDef.version end
